...
Mcp__OAuth__Resource=https://mcp.example.com/mcp
...
| Key Vault | `kv` | `kv-ymcp-prod-aue-<unique>` |
...
Assume the public endpoint is configurable, using `mcp.example.com` only as documentation data.
...
```powershell
Invoke-WebRequest https://mcp.example.com/health/live
Invoke-WebRequest https://mcp.example.com/health/ready
```
...
- Use `mcp.example.com` only as an example, never as a runtime default. Require the hostname as a Terraform/application variable.
