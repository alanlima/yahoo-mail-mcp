...
2. In ChatGPT's connector/developer-mode MCP setup, enter `https://mcp.alanlima.cloud/mcp` as the server URL and choose OAuth.
3. Choose user-provided OAuth client credentials. Retrieve `mcp-oauth-client-id` and `mcp-oauth-client-secret` from `kv-ymcp-prod-aue-01` in a private shell and paste them only into ChatGPT's credential fields.
4. Request `openid profile offline_access https://mcp.alanlima.cloud/mcp/access_as_user`.
...
```text
Authorization URL: https://login.microsoftonline.com/b16c65ab-9711-44cd-abf1-24e6180a4609/oauth2/v2.0/authorize
Token URL:         https://login.microsoftonline.com/b16c65ab-9711-44cd-abf1-24e6180a4609/oauth2/v2.0/token
Client ID:         7553b3c9-f602-42a8-b5fe-8a54af0dd3af
```
...
```powershell
az keyvault secret show --vault-name kv-ymcp-prod-aue-01 --name mcp-oauth-client-id --query value --output tsv
az keyvault secret show --vault-name kv-ymcp-prod-aue-01 --name mcp-oauth-client-secret --query value --output tsv
```
