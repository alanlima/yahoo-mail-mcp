...
- The state principal needs `s3:ListBucket` on the configured state bucket for the `yahoo-mail-mcp/prod/` prefix and `s3:GetObject`, `s3:PutObject`, and `s3:DeleteObject` on both state and `.tflock` objects in that prefix. Bucket versioning and server-side encryption should already be enabled.
- A Yahoo app password. Never use the normal Yahoo account password.

The two state objects are:

```text
s3://example-terraform-state-bucket/yahoo-mail-mcp/prod/foundation.tfstate
s3://example-terraform-state-bucket/yahoo-mail-mcp/prod/application.tfstate
```
...
```powershell
$tfStateBucket = '<terraform-state-bucket>'
...
The script creates a single-tenant Entra app, exposes the `access_as_user` delegated scope, creates its service principal and a 180-day client credential, and writes only `mcp-oauth-client-id`, `mcp-oauth-client-secret`, and `mcp-oauth-tenant-id` to Key Vault. It does not print the client secret.

Set `mcp_oauth_enabled=true`, the tenant GUID, and `mcp_oauth_audience` to the application GUID in the ignored production application variables. The full resource URL remains the requested OAuth scope prefix. The Container App reads the client ID through a Key Vault reference; it intentionally does not load the OAuth client secret.

OAuth may also be enabled in Development for debugging. The checked-in Development
settings keep OAuth disabled and contain no deployment identifiers; supply local
values through user-secrets or environment variables.
...
An AWS Certificate Manager certificate cannot be referenced directly by Azure Container Apps. Azure would require an exportable certificate plus its private key uploaded as a PFX. Because this design uses an Azure-managed certificate after Route 53 validation, the ACM certificate is not copied or exposed.
...
```powershell
Invoke-WebRequest 'https://mcp.example.com/.well-known/oauth-protected-resource'
```

After replacing the example hostname, the response must identify the configured MCP URL as the resource, the tenant-specific Entra v2 authorization server, and the qualified `access_as_user` scope. The final interactive authorization-code flow can be verified only from ChatGPT.
...
MCP_OAUTH_CLIENT_ID
AWS_ROLE_TO_ASSUME, AWS_REGION, TF_STATE_BUCKET
