...
- Terraform uses distinct encrypted S3 state keys in `lima-terraform-states`, native lock files, managed identity, versionless Key Vault references, ACR admin disabled, one replica, Route 53 records, and a managed certificate binding.
...
- The existing S3 bucket `lima-terraform-states` is initialized with the separate lowercase production state keys.
- The Entra app registration and service principal exist; its client ID and 180-day secret were written directly to Key Vault without printing the secret.
- Route 53 validation succeeded, the Azure-managed certificate for `mcp.alanlima.cloud` reached `Succeeded`, the custom hostname is SNI-bound, and public `/health/live` returns HTTP 200.
